...
def local_model(monkeypatch):
    monkeypatch.setattr(service,'MODEL','hf.co/unsloth/Qwen3.8-27B-GGUF:UD-IQ3_S')
...
def test_api_local_y_plan_valido(monkeypatch,local_model):
    def post(self,url,json):
        ...
        assert json['model']=='hf.co/unsloth/Qwen3.8-27B-GGUF:UD-IQ3_S'
    ...
    assert model=='hf.co/unsloth/Qwen3.8-27B-GGUF:UD-IQ3_S'
...
def test_errores_http_controlados(monkeypatch,local_model,status):
    ...
    if status==404:assert 'ollama pull hf.co/unsloth/Qwen3.8-27B-GGUF:UD-IQ3_S' in error.value.detail
