...
import {mobileReminders,startReminders} from './mobile-notifications.js';
...
export async function api(path,options={}) {
 if(path==='/auth/logout'||path==='/auth/login')await mobileReminders?.stop();
 const {status,data}=await request(path,options);
 if(status<200||status>=300){if(status===401){await mobileReminders?.stop();if(!path.startsWith('/auth/'))location.href='login.html';}const error=new Error(data?.detail||'No pudimos completar la solicitud. Inténtalo nuevamente.');error.status=status;throw error;}
 if(options.method&&options.method.toUpperCase()!=='GET'&&/^\/(tareas|evaluaciones|planes)(\/|$)/.test(path)){
  await mobileReminders?.sync().catch(()=>toast('Cambio guardado. No se pudieron actualizar los avisos del teléfono; abre Notificaciones para reintentarlo.',true));
 }
 return data;
}
